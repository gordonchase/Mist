...
void FixedUpdate()
{
    ...
    // Continuous push/pull if target selected
Vector2 dir = (selectedTarget.position - transform.position).normalized;

if (selectedTarget != null)
{
    Rigidbody2D rbMetal = selectedTarget.GetComponent<Rigidbody2D>();
    Vector2 direction = (selectedTarget.position - transform.position).normalized; // renamed from 'dir'
    float distance = Vector2.Distance(selectedTarget.position, transform.position);
    float maxDistance = metalDetectRange; // max range for scaling

    float scaledPull = pullForce * Mathf.Clamp01(1f - (distance / maxDistance));
    float scaledPush = pushForce * Mathf.Clamp01(1f - (distance / maxDistance));

    // Pulling iron
    if (buringiron)
    {
        if (rbMetal != null)
            rbMetal.AddForce(-direction * scaledPull * Time.fixedDeltaTime);
        rb.AddForce(direction * scaledPull * Time.fixedDeltaTime); // Player pulled toward metal
    }
    ...
    {
        if (rbMetal != null)
            rbMetal.AddForce(direction * scaledPush * Time.fixedDeltaTime);
        rb.AddForce(-direction * scaledPush * Time.fixedDeltaTime); // Player pushed backward
    }
}
}
...
void HandleLineChooserInput()
{
    ...
    if (Input.GetKeyDown(KeyCode.W))
    {
        ...
        selectedTarget = metalTargets[selectedIndex];
    }
