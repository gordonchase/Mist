...
public class PlayerController : MonoBehaviour
{
    ...
    IEnumerator DoEverySecond()
    {
        while (true)
        {
            ...
            {
                if (flaring) tinbarpercent -= 2;
                else tinbarpercent -= 1;
            }
            if (buringpewter && pewterbarpercent>0)
            {
                if (flaring) pewterbarpercent -= 2;
                else pewterbarpercent -= 1;
            }
            if (buringsteel && steelbarpercent>0)
            {
                if (flaring) steelbarpercent -= 2;
                else steelbarpercent -= 1;
            }
            if (buringiron && ironbarpercent>0)
            {
                if (flaring) ironbarpercent -= 2;
                else ironbarpercent -= 1;
            }
    ...
    void Update()
    {
        ...
        {
            if (buringsteel) buringsteel = false;
            else buringsteel = true;
        }
        if (Input.GetKeyDown(KeyCode.V))
        {
            if (buringiron) buringiron = false;
            else buringiron = true;
        }

        if (Input.GetKeyDown(KeyCode.F))
        {
            if (flaring) flaring = false;
            else flaring = true;
        }

        // ===== NEW: Activate line chooser for steel/iron =====
        if (Input.GetKeyDown(KeyCode.C) && buringsteel)
        {
            ActivateLineChooser();
        }
        if (Input.GetKeyDown(KeyCode.V) && buringiron)
        {
            ActivateLineChooser();
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        // ===== NEW: continuous pull/push if a target is selected =====
        if (selectedTarget != null)
        {
            ...
            if (rbMetal != null)
            {
                ...
                rbMetal.AddForce(-dir * pullStrength * Time.fixedDeltaTime);
            }
        }
    }
    ...
    private float pullStrength = 500f; // force strength
    ...
    void HandleLineChooserInput()
    {
        ...
        if (Input.GetKeyDown(KeyCode.W))
        {
            ...
            Debug.Log("Selected metal: " + selectedTarget.name);
        }
